...
void devolver_INFO(string NombreUser, string UserStatus, int lola){ 
...
void SendKey(int connectfd,char *buf,struct sockaddr_in socketcliente){
	read( connectfd , buf, PORT);
	ClientMessage * message(new ClientMessage);
	message->ParseFromString(buf);
	key = message->synchronize().ip();
	Devolver_usuario(now, now1, now2, now3, now4);
}

void SendKey2(int connectfd,char *buf,struct sockaddr_in socketcliente){
	read( connectfd , buf, PORT);
	ClientMessage * message(new ClientMessage);
	message->ParseFromString(buf);
	key2 = message->synchronize().ip();
}
...
void *conexionConClientes(void *args)
{
	...
	if (socket_cliente > 0)
	{
		...
		idnow = lola;
		if(key2 == now){
		devolver_INFO(NombreUser, UserStatus, lola);
		}
	    } else if(m == 1){
		now1 = NombreUser;
